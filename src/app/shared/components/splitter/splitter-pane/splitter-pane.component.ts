...
export class SplitterPaneComponent implements OnChanges, AfterViewChecked {
  ...
  set size(newSize) {
    ...
    const paneFixedClass = 'app-splitter-pane-fixed';
    if (this.size) {
  ...
  @HostBinding('class.app-splitter-pane') paneClass = true;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      (changes.collapsible && !changes.collapsible.isFirstChange()) ||
  ...
  // 收起时隐藏当前pane
  toggleCollapseClass() {
    const paneHiddenClass = 'app-splitter-pane-hidden';
    if (!this.collapsed) {
  ...
  toggleNearPaneFlexGrow() {
    const flexGrowClass = 'app-splitter-pane-grow';
  ...
  ngAfterViewChecked() {
    ...
    if (!this.size && !this.resizable) {
      this.renderer.addClass(this.element, 'app-splitter-pane-fixed');
    }
  }
}
